...
public class TreeHelper {
     ...
     */
    public static <T extends BaseNodeBean> List<Node> getSortedNodes(
     ...
     * @throws IllegalAccessException
     */
    public static <T extends BaseNodeBean> List<Node> convertData2Nodes(List<T> datas, boolean isHide)
            throws IllegalAccessException, IllegalArgumentException {
    ...
    public static void setNodeIcon(Node node) {
        if (node.getChildrenNodes().size() > 0 && node.isExpand()) {
            node.setIcon(R.drawable.xiaji_pass);
        } else if (node.getChildrenNodes().size() > 0 && !node.isExpand()) {
            node.setIcon(R.drawable.xiaji);
        } else
            node.setIcon(R.drawable.xiaji_pass);
    }
