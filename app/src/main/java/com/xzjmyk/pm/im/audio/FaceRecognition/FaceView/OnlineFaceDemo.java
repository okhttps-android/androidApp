package com.xzjmyk.pm.im.audio.FaceRecognition.FaceView;
...
import com.xzjmyk.pm.im.audio.FaceRecognition.Util.FaceUtil;
 ...
 * 在线人脸示例
 */
public class OnlineFaceDemo extends BaseActivity implements OnClickListener {
	...
	private void register(JSONObject obj) throws JSONException {
		...
		if (ret != 0) {
			showTip("注册失败");
			LogUtil.prinlnLongMsg("FaceobjData","detect:"+obj.toString());
			return;
		}
		...
		LogUtil.prinlnLongMsg("FaceobjData","register:"+obj.toString());
	}
	...
	private void verify(JSONObject obj) throws JSONException {
		...
		if (ret != 0) {
			showTip("验证失败");
			LogUtil.prinlnLongMsg("FaceobjData","verify:"+obj.toString());
			return;
		}
		if ("success".equals(obj.get("rst"))) {
	...
	private void align(JSONObject obj) throws JSONException {
		...
		LogUtil.prinlnLongMsg("FaceobjData","align:"+obj.toString());
	}

	//请求返回消息处理，和Handler一样的
	...
	public void onClick(View view) {
		...
		case R.id.online_reg:
			...
			if (null != mImageData) {
				...
				mProDialog.show();
				// 设置用户标识，格式为6-18个字符（由字母、数字、下划线组成，不能包含空格）。
