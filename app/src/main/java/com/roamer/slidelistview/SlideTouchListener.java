...
public class SlideTouchListener implements OnTouchListener {
	...
	private void autoScroll(final int offset, final boolean toOpen) {
		...
		animate(mSlideItem.frontView).translationX(moveTo);
		animate(mSlideItem.frontView).setDuration(getAnimationTime());
		/*ViewPropertyAnimatorCompat viewPropertyAnimatorCompat = (ViewPropertyAnimatorCompat) animate(mSlideItem.frontView).setListener(new AnimatorListenerAdapter() {
			@Override
